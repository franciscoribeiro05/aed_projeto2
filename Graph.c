...
Graph* GraphCreateTranspose(const Graph* g) {
  ...
  // COMPLETE THE CODE
    ...
    for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(originalVertices), i++) {
        struct _Vertex* v = ListGetCurrentItem(originalVertices);
        List* edges = v->edgesList;

        // Iterar pelas arestas do vértice atual
        if (!ListIsEmpty(edges)) {
            ListMoveToHead(edges);
            unsigned int numEdges = ListGetSize(edges);
            for (unsigned int j = 0; j < numEdges; ListMoveToNext(edges), j++) {
                struct _Edge* edge = ListGetCurrentItem(edges);

                // Adicionar a aresta invertida no grafo transposto
                if (g->isWeighted) {
                    GraphAddWeightedEdge(transpose, edge->adjVertex, v->id, edge->weight);
                } else {
                    GraphAddEdge(transpose, edge->adjVertex, v->id);
                }
            }
        }
    }
